...
public class Walker {
     ...
     */
    private mining.Walker.Type getType(String s) {
        if (s.matches("(?i)((.*)?door)")) {
            return mining.Walker.Type.DOOR;
        } else if (s.matches("(?i)(gate(.*)?)")) {
            return mining.Walker.Type.GATE;
        } else if (s.equalsIgnoreCase("ladder")) {
            return mining.Walker.Type.LADDER;
        }
        return mining.Walker.Type.TEAPOT;
    }
